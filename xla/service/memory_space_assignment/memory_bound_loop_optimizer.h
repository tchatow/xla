...
struct LoopOptimizerChunkInterval {
  ...
  std::string ToString() const {
    return absl::StrFormat(
        ...
        "chunk: %s",
        begin_idx_in_loop, end_idx_in_loop, chunks.first.value().ToString(),
...
class LoopOptimizerBestFitHeap
 ...
 private:
  ...
  // allocation_blocks_.
  AllocationBlock* GetAllocationBlock(int64_t start_time, int64_t end_time,
                                      int64_t size);
  ...
  // AllocationBlock.
  AllocationBlock* CreateSameEvenAndOddAllocationBlock(
      int64_t begin_idx_in_loop, int64_t end_idx_in_loop, int64_t size);

  // Creates colocated allocation blocks for loop interval [begin_idx_in_loop,
  // end_idx_in_loop] in 0th and 2nd loop iterations and returns one
  // AllocationBlock.
  AllocationBlock* CreateEvenAllocationBlock(int64_t begin_idx_in_loop,
                                             int64_t end_idx_in_loop,
                                             int64_t size);
  // Creates colocated allocation blocks for loop interval [begin_idx_in_loop,
  // end_idx_in_loop] in 1st and 3rd loop iterations and returns one
  // AllocationBlock.
  AllocationBlock* CreateOddAllocationBlock(int64_t begin_idx_in_loop,
                                            int64_t end_idx_in_loop,
                                            int64_t size);

  // Creates a pair of colocated allocation blocks for loop interval
  // [begin_idx_in_loop, end_idx_in_loop] in 0st and 2nd loop iterations and
  // another pair of colocated allocation blocks in 1st and 3rd loop iterations
  // and returns one allocation block from each pair.
