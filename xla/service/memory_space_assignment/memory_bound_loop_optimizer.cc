...
AllocationBlock* LoopOptimizerBestFitHeap::GetAllocationBlock(
...
AllocationBlock* LoopOptimizerBestFitHeap::CreateEvenAllocationBlock(
  ...
  AllocationBlock* second_allocation_block =
...
AllocationBlock* LoopOptimizerBestFitHeap::CreateOddAllocationBlock(
    int64_t begin_idx_in_loop, int64_t end_idx_in_loop, int64_t size) {
  AllocationBlock* first_allocation_block = GetAllocationBlock(
...
LoopOptimizerBestFitHeap::CreateEvenAndOddAllocationBlock(
    int64_t begin_idx_in_loop, int64_t end_idx_in_loop, int64_t size) {
...
EvenOddChunkPair LoopOptimizerBestFitHeap::FindEvenAndOddAllocationBetween(
  ...
  auto [even_allocation, odd_allocation] =
      CreateEvenAndOddAllocationBlock(begin_idx_in_loop, end_idx_in_loop, size);
...
AllocationBlock* LoopOptimizerBestFitHeap::CreateSameEvenAndOddAllocationBlock(
    int64_t begin_idx_in_loop, int64_t end_idx_in_loop, int64_t size) {
  AllocationBlock* first_allocation_block =
      GetAllocationBlock(begin_idx_in_loop, end_idx_in_loop, size);
  CreateBufferInterval(first_allocation_block);
  ...
  CreateBufferInterval(second_allocation_block, first_allocation_block);
  AllocationBlock* third_allocation_block =
      GetAllocationBlock(begin_idx_in_loop + 2 * loop_size_,
                         end_idx_in_loop + 2 * loop_size_, size);
  CreateBufferInterval(third_allocation_block, first_allocation_block);
  AllocationBlock* fourth_allocation_block =
      GetAllocationBlock(begin_idx_in_loop + 3 * loop_size_,
                         end_idx_in_loop + 3 * loop_size_, size);
...
EvenOddChunkPair LoopOptimizerBestFitHeap::FindSameEvenAndOddAllocationBetween(
  ...
  auto allocation = CreateSameEvenAndOddAllocationBlock(begin_idx_in_loop,
                                                        end_idx_in_loop, size);
  std::optional<HeapSimulator::Chunk> chunk =
  ...
  for (int i = first_use_idx - 1; i >= last_use_idx_sentinel - loop_size_;
    ...
    if (i < 0) {
      for (; context.value_indices[early_forced_prefetch_value_search_index] !=
        ...
        // early force it.
        if (early_forced_value->alternate_memory_begin_idx_in_loop.value() <
            0) {
          break;
        }
