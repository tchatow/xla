...
void MemoryBoundLoopOptimizer::LoopValue::SetChunkPairAndInterval(
    EvenOddChunkPair chunk_pair, int64_t start_time, int64_t end_time) {
  chunks = chunk_pair;
...
bool MemoryBoundLoopOptimizer::AllocateTemporary(LoopValue& value) {
  ...
  EvenOddChunkPair chunks =
      heap_.AllocateSameEvenAndOddBetween(start_time, end_time, value.size);
  if (!chunks.first.has_value() || !chunks.second.has_value()) {
    VLOG(3) << "Could not find Allocation for temporary value: "
            << value.ToString();
    return false;
  }
  value.SetChunkPairAndInterval(chunks, start_time, end_time);
...
bool MemoryBoundLoopOptimizer::AllocatePrefetch(
  ...
  for (int current_idx = first_use_idx - 1;
    ...
    if (current_idx < 0) {
      for (; context.value_indices[early_forced_prefetch_value_search_index] !=
        ...
        // early force it.
        if (early_forced_value->alternate_memory_start_time.value() < 0) {
          break;
        }
    ...
    VLOG(3) << "Memory usage before adding pending chunks: \n"
            << heap_.MemoryUsageToAsciiArt();
    std::vector<ChunkInterval> pending_chunk_intervals;
    for (int early_forced_prefetch_value_index :
      ...
      pending_chunk_intervals.push_back({start_time, end_time, chunks});
    ...
    if (pending_chunk_intervals.size() ==
        early_forced_prefetch_value_indices.size()) {
      int64_t start_time = current_idx;
    ...
    for (auto pending_chunk_interval : pending_chunk_intervals) {
      ...
      heap_.RemoveEvenOddChunkPair(pending_chunk_interval.start_time,
                                   pending_chunk_interval.end_time,
  ...
  for (int early_forced_prefetch_value_index :
    ...
    CopyAllocation* early_forced_prefetch = static_cast<CopyAllocation*>(
        early_forced_value->allocations.back().get());
    int64_t start_time = positive_copy_start_time;
    int64_t end_time = early_forced_value->alternate_memory_end_time.value();
    EvenOddChunkPair chunks = heap_.AllocateEvenAndOddBetween(
        start_time, end_time, early_forced_value->size);
    ...
    CHECK_LT(start_time,
             early_forced_value->alternate_memory_start_time.value());
    early_forced_value->SetChunkPairAndInterval(chunks, start_time, end_time);
