...
class LoopOptimizerBestFitHeapTest : public ::testing::Test {
 public:
  ...
  bool IsAllocateSameEvenAndOddBetweenSuccessful(int64_t begin_idx,
                                                 int64_t end_idx,
                                                 int64_t size) {
    EvenOddChunkPair chunks =
        heap_.AllocateSameEvenAndOddBetween(begin_idx, end_idx, size);
    return chunks.first.has_value() && chunks.second.has_value();
  }

  bool CanFindSameEvenAndOddAllocationBetween(int64_t begin_idx,
                                              int64_t end_idx, int64_t size) {
    EvenOddChunkPair chunks =
        heap_.FindSameEvenAndOddAllocationBetween(begin_idx, end_idx, size);
    return chunks.first.has_value() && chunks.second.has_value();
  }

  bool IsAllocateEvenAndOddBetweenSuccessful(int64_t begin_idx, int64_t end_idx,
                                             int64_t size) {
    EvenOddChunkPair chunks =
        heap_.AllocateEvenAndOddBetween(begin_idx, end_idx, size);
    return chunks.first.has_value() && chunks.second.has_value();
  }

  bool CanFindEvenAndOddAllocationBetween(int64_t begin_idx, int64_t end_idx,
                                          int64_t size) {
    EvenOddChunkPair chunks =
        heap_.FindEvenAndOddAllocationBetween(begin_idx, end_idx, size);
    return chunks.first.has_value() && chunks.second.has_value();
  }
