...
func.func @vector_extract(%arg0: vector<2x3xf32>,
    %arg1: index, %arg2: index) -> f32 {
  %v = vector.extract %arg0[%arg1, %arg2] : f32 from vector<2x3xf32>
  func.return %v : f32
}
// CHECK: #[[$MAP:.+]] = #xla_gpu.indexing_map<(d0, d1) -> (d0 * 3 + d1), domain: d0 in [0, 1], d1 in [0, 2]>

// CHECK-LABEL: func.func @vector_extract(
// CHECK-SAME:      %[[SRC:.*]]: vector<6xf32>,
// CHECK-SAME:      %[[I:.*]]: index, %[[J:.*]]: index) -> f32 {
// CHECK:        %[[INDEX:.*]] = xla_gpu.apply_indexing #[[$MAP]](%[[I]], %[[J]])
...
func.func @vector_insert(%arg0: vector<10x24xf32>) -> vector<10x24xf32> {
  %c1 = arith.constant 1 : index
  %scalar = arith.constant 3.0 : f32
  %out = vector.insert %scalar, %arg0 [%c1, %c1] : f32 into vector<10x24xf32>
...
func.func @for_loop_vector(%t0: vector<32x1024xf32>, %t1: vector<64x8x4xf32>)
  ...
  %for:2 = scf.for %i = %c0 to %c64 step %c32 iter_args(%t0_ = %t0, %t1_ = %t1)
    ...
    %update0 = vector.insert %c0_f32, %t0_ [%c1, %i] : f32 into vector<32x1024xf32>
    %update1 = vector.insert %c0_f32, %t1_ [%i, %c1, %c1] : f32 into vector<64x8x4xf32>
    scf.yield %update0, %update1 : vector<32x1024xf32>, vector<64x8x4xf32>
  } {some_attr}
    return %for#0, %for#1, %c0_f32 : vector<32x1024xf32>, vector<64x8x4xf32>, f32
}
// CHECK: #[[$MAP0:.+]] = #xla_gpu.indexing_map<(d0) -> (d0 + 1024)
// CHECK: #[[$MAP1:.+]] = #xla_gpu.indexing_map<(d0) -> (d0 * 32 + 5)
// CHECK-LABEL: func.func @for_loop_vector(
// CHECK-SAME:      %[[V0:.*]]: vector<32768xf32>,
// CHECK-SAME:      %[[V1:.*]]: vector<2048xf32>) -> (vector<32768xf32>, vector<2048xf32>, f32) {
...
// CHECK:        scf.yield %[[UPD0]], %[[UPD1]] : vector<32768xf32>, vector<2048xf32>

// -----

func.func @if_op_vector(%arg0: vector<8x1xf32>, %value: f32) -> vector<8x1xf32> {
  %c0 = arith.constant 0 : index
  %c1 = arith.constant 1 : index
  ...
  %0 = scf.for %arg2 = %c0 to %c8 step %c1 iter_args(%arg3 = %cst) -> (vector<8x1xf32>) {
